...
export function AddRosterModal(props) {
  ...
  const { data: factions } = useSWR(
    `/faction.php?${teamParams}`,
    !isNil(settings.edition) ? fetchFactions : null,
  );
  ...
  const teamOptions = find(factions, { factionid: form?.getValues()?.faction })
    ?.killteams?.filter((team) => !team.isCustom)
    ?.map((team) => ({
      ...
      <form onSubmit={handleCreateRoster}>
        <Stack>
          ...
          <Group justify="flex-end">
            <Button
              variant="default"
              onClick={() => modals.close("create-roster")}
            >
              Cancel
            </Button>
