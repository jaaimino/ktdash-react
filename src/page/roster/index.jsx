...
export default function Roster() {
      ...
      children: (
        <div
          dangerouslySetInnerHTML={{
            __html: `${roster?.killteam?.killteamcomp}`,
          }}
        />
      ),
    });
  ...
  const handleShowReOrderOperatives = React.useCallback(() => {
    modals.open({
      ...
      children: (
        <OrderOperativesModal
          roster={roster}
          onClose={handleReOrderOperatives}
        />
      ),
    });
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [roster, isSmallScreen]);
  const handleReOrderOperatives = React.useCallback(
    (operatives) => {
  ...
  const handleAddOperative = React.useCallback(
    (operative) => {
      const newOperative = {
  ...
  const handleEditOperative = React.useCallback(
    (operative) => {
        ...
        if (data?.rosteropid) {
          ...
          setRoster({
            ...
            operatives: roster.operatives?.map((op) =>
              op.rosteropid === data.rosteropid ? data : op,
            ),
          });
        }
      });
    ...
    request(`/rosteroperative.php?roid=${operative.rosteropid}`, {
      ...
      if (data?.success) {
        ...
        setRoster({
          ...roster,
          operatives: [
            ...roster?.operatives?.filter(
              (op) => op.rosteropid !== operative.rosteropid,
            ),
          ],
        });
      }
  ...
  React.useEffect(() => {
    ...
    isSmallScreen,
    handleAddOperative,
    handleShowUpdateRosterPortrait,
    handleCopyRoster,
    handleShowReOrderOperatives,
    handleReOrderOperatives,
  ]);
            ...
            <Group>
              <Text>
                <Link href={`/fa/${roster.factionid}/kt/${roster.killteamid}`}>
                  {roster.killteamname}
                  <sup>{roster.edition}</sup>
                </Link>{" "}
                by <Link href={`/u/${roster.username}`}>{roster.username}</Link>
              </Text>
            </Group>
            {!!roster.notes && <Text>{roster.notes}</Text>}
            {!roster.notes && (
              <div
                dangerouslySetInnerHTML={{
                  __html: `${roster.killteamdescription}`,
                }}
              />
            )}
          </Stack>
        ...
        <SimpleGrid cols={{ base: 1, sm: 2, lg: 3, xl: 4 }} spacing="md">
          {roster?.operatives?.map((operative, index) => (
            <OperativeCard
              ...
              editable={canEdit}
              operative={operative}
              onEdit={handleShowEditOperative}
              onDelete={handleConfirmDeleteOperative}
              edition={roster.edition}
            />
          ))}
        </SimpleGrid>
      </Stack>
