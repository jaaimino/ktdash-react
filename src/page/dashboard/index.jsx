...
export default function Dashboard() {
  ...
  const { data: roster, mutate: setRoster } = useSWR(
    ["/roster.php", rosterId],
    fetchRoster
  );
  const router = useRouter();
  const [canEdit, setCanEdit] = useState(userData?.username === roster?.username)
  ...
  const groupedEquipment = groupBy(
    roster?.rostereqs?.filter((equip) => !isNarrativeEquipment(equip)),
    "eqcategory"
  );
  const handleResetDashboard = React.useCallback(() => {
    request(
      `/rosterreset.php?rid=${roster?.rosterid}&VP=${settings?.startvp}&CP=${settings?.startcp}`,
      {
        method: "POST",
      }
    ).then(
  ...
  const handleUpdateOperatives = React.useCallback(
    (operatives) => {
  ...
  const handleShowSelectOperatives = React.useCallback(() => {
    modals.open({
      ...
      children: (
        <SelectOperativesModal
          roster={roster}
          onClose={handleUpdateOperatives}
        />
      ),
    });
    // eslint-disable-next-line react-hooks/exhaustive-deps
  ...
  // eslint-disable-next-line react-hooks/exhaustive-deps
  const handleSaveUpdateRoster = React.useCallback(
    debounce((newRoster) => {
      request(`/roster.php`, {
        method: "POST",
  ...
  const handleQuickUpdateRoster = React.useCallback(
    (updates) => {
      const newRoster = {
  ...
  const handleSelectPloy = React.useCallback(
    (ploy, checked) => {
      const newPloys = checked
        ? [...roster.ployids.split(",").filter((id) => !!id), ploy.ployid]
        : [
            ...roster.ployids
              .split(",")
              .filter((id) => !!id)
              .filter((pl) => pl !== ploy.ployid),
          ];
      const newRoster = {
        userid: roster.userid,
        rosterid: roster.rosterid,
  ...
  const handleUpdateTacOp = React.useCallback(
    (tacop) => {
      const newTacOp = {
        userid: roster.userid,
        rosterid: roster.rosterid,
      ...
      setRoster(
        {
          ...roster,
          tacops: [
            ...roster.tacops.map((op) =>
              op.tacopid === tacop.tacopid ? tacop : op
            ),
          ],
        },
        { optimisticData: true, revalidate: false }
        ...
        setRoster(
          {
            ...roster,
            rostereqs: [
              ...roster?.rostereqs?.map((eq) =>
                eq.eqid === equipment.eqid ? { ...eq, selected: 1 } : eq
              ),
            ],
          },
          { optimisticData: true, revalidate: false }
        ...
        setRoster(
          {
            ...
            rostereqs: [
              ...roster?.rostereqs?.map((eq) =>
                eq.eqid === equipment.eqid ? { ...eq, selected: 0 } : eq
              ),
            ],
          },
          { optimisticData: true, revalidate: false }
  ...
  const handleUpdateOperativeWounds = React.useCallback(
    (operative, wounds) => {
      request(`/rosteropw.php?roid=${operative.rosteropid}&curW=${wounds}`, {
        ...
        if (data?.success) {
          setRoster(
            {
              ...roster,
              operatives: roster.operatives?.map((op) =>
                op.rosteropid === operative.rosteropid
                  ? {
                      ...op,
                      curW: wounds,
                    }
  ...
  const handleUpdateOperativeOrder = React.useCallback(
    (operative, newoporder, newactivated) => {
      setRoster(
        {
          ...roster,
          operatives: roster.operatives?.map((op) =>
      ...
      request(
        `/rosteroporder.php?roid=${operative.rosteropid}&order=${newoporder}`,
        {
          method: "POST",
        }
      );
      request(
        `/rosteropactivated.php?roid=${operative.rosteropid}&activated=${newactivated}`,
        {
          method: "POST",
        }
      );
  ...
  React.useEffect(() => {
    ...
    useEffect(() => {
        ...
        if (userData?.username === roster?.username) setCanEdit(true)
    }, [roster?.username, session, userData.username]);

  if (!rosterId) {
    return (
      <Container py="md" px="md" fluid>
        <>
          No roster selected. Go to{" "}
          <Link href={`/u/${userData?.username}`}>your rosters</Link> and deploy
          one.
        </>
      </Container>
  ...
  if (!roster?.rosterid) {
    return (
      <Container py="md" px="md" fluid>
        <>
          Failed to load roster {rosterId}. Go to{" "}
          <Link href={`/u/${userData?.username}`}>your rosters</Link> and deploy
          a different one.
        </>
      </Container>
      ...
      <Stack>
        <Card>
          <SimpleGrid cols={{ base: 3 }} spacing={0} py="sm">
            <Stack justify="center" align="center" gap="xs">
              <Title
                style={{
                  borderBottom: "2px solid var(--mantine-color-orange-8)",
                }}
                order={3}
              >
                CP
              ...
              <Group gap="xs">
                <ActionIcon
                  onClick={() =>
                    roster.CP !== 0
                      ? handleQuickUpdateRoster({ CP: roster.CP - 1 })
                      : () => {}
                  }
                  variant="default"
                >
                ...
                {roster.CP.toString()}
                <ActionIcon
                  onClick={() => handleQuickUpdateRoster({ CP: roster.CP + 1 })}
                  variant="default"
                >
                  <IconPlus />
                </ActionIcon>
            ...
            <Stack justify="center" align="center" gap="xs">
              ...
              <Group gap="xs">
                <ActionIcon
                  onClick={() =>
                    roster.TP !== 0
                      ? handleQuickUpdateRoster({ TP: roster.TP - 1 })
                      : () => {}
                  }
                  variant="default"
                >
                ...
                {roster.TP.toString()}
                <ActionIcon
                  onClick={() => handleQuickUpdateRoster({ TP: roster.TP + 1 })}
                  variant="default"
                >
                  <IconPlus />
                </ActionIcon>
            ...
            <Stack justify="center" align="center" gap="xs">
              ...
              <Group gap="xs">
                <ActionIcon
                  onClick={() =>
                    roster.VP !== 0
                      ? handleQuickUpdateRoster({ VP: roster.VP - 1 })
                      : () => {}
                  }
                  variant="default"
                >
                ...
                {roster.VP.toString()}
                <ActionIcon
                  onClick={() => handleQuickUpdateRoster({ VP: roster.VP + 1 })}
                  variant="default"
                >
                  <IconPlus />
                </ActionIcon>
        ...
        <Tabs defaultValue="operatives">
              ...
              {roster?.operatives
                ...
                ?.map((operative, index) => (
                  <OperativeCard
                    ...
                    collapsible
                    woundTracker
                    onUpdateWounds={(wounds) =>
                      handleUpdateOperativeWounds(operative, wounds)
                    }
                    orderTracker
                    onUpdateOrder={(newoporder, newactivated) =>
                      handleUpdateOperativeOrder(
                        operative,
                        newoporder,
                        newactivated
                      )
          ...
          <Tabs.Panel value="ploys">
            <PloyCards
              selectable
              selectedPloys={roster.ployids}
              onSelect={handleSelectPloy}
              killteam={killteam}
            />
          </Tabs.Panel>
          {killteam.edition === "kt24" && (
          ...
          <Tabs.Panel value="tacops">
            <TacOpCards
              selectable
              tacops={roster.tacops}
              onUpdate={handleUpdateTacOp}
            />
          </Tabs.Panel>
        </Tabs>
      </Stack>
